...
export default function SegmentedControl({ options, value, onChange, size = 'md', label }) {
  ...
  return (
        <div
          role="radiogroup"
          aria-label={label}
          className="flex w-fit max-w-full flex-nowrap overflow-x-auto border border-line-strong"
        >      
        {options.map((option) => {
        ...
        return (
          <button
           ...
           className={[
            'h-10 shrink-0 cursor-pointer whitespace-nowrap border-0 border-r border-line-strong font-bold uppercase',
